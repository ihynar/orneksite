...
async function seed() {
  ...
  if (Number(servicesCount[0].count) === 0) {
    for (const s of servicesSeed) {
      await pool.query("INSERT INTO services (title, description, price) VALUES ($1,$2,$3)", [
        s.title,
        s.description,
        s.price,
      ]);
  ...
  if (Number(galleryCount[0].count) === 0) {
    for (const g of gallerySeed) {
      await pool.query("INSERT INTO gallery_items (title, description, image_url) VALUES ($1,$2,$3)", [
        g.title,
        g.description,
        g.image_url,
      ]);
